...
#include "helicopter_flight_simulation.hh"
...
class gz::sim::systems::HelicopterFlightSimulationPrivate
{
...
public:
  gz::math::Vector3d cp = gz::math::Vector3d::Zero;

  /// \brief Normally, this is taken as a direction parallel to the chord
  /// of the airfoil in zero angle of attack forward flight.
public:
  gz::math::Vector3d forward = gz::math::Vector3d::UnitX;
...
public:
  gz::math::Vector3d upward = gz::math::Vector3d::UnitZ;
...
public:
  ...
  /// \brief Gazebo Transport node for publishing wrench data
public:
  gz::transport::Node node;

  /// \brief Publisher for the wrench data
public:
  gz::transport::Node::Publisher wrenchPub;

private:
  /// \brief Latest angle of attack value received from the subscriber
  double currentAlpha = 0.1;

  /// \brief Mutex to protect access to the currentAlpha variable
  std::mutex alphaMutex;

  /// \brief Cyclic control inputs
public:
  double alphaC = 0.0; // Corresponding to \theta_C (longitudinal cyclic)
  double alphaS = 0.0; // Corresponding to \theta_S (lateral cyclic)

public:
  /// \brief Callback function for the angle of attack subscription
  void OnAngleOfAttackMsg(const gz::msgs::Double &_msg)
  {
    std::lock_guard<std::mutex> lock(this->alphaMutex);
    this->currentAlpha = _msg.data();
  }
  void OnAlphaCMsg(const gz::msgs::Double &_msg)
  {
    std::lock_guard<std::mutex> lock(this->alphaMutex);
    this->alphaC = _msg.data();
  }

  void OnAlphaSMsg(const gz::msgs::Double &_msg)
  {
    std::lock_guard<std::mutex> lock(this->alphaMutex);
    this->alphaS = _msg.data();
  }
};

//////////////////////////////////////////////////
void HelicopterFlightSimulationPrivate::Load(const EntityComponentManager &_ecm,
                                             const sdf::ElementPtr &_sdf)
{
  // Initialize the wrench publisher
  this->wrenchPub = this->node.Advertise<gz::msgs::Wrench>(
      "/wrench/" + this->model.Name(_ecm) + "/" + _sdf->Get<std::string>("link_name"));

  this->node.Subscribe("/angle_of_attack", &HelicopterFlightSimulationPrivate::OnAngleOfAttackMsg, this);
  // Subscribe to cyclic control topics
  this->node.Subscribe("/alpha_c", &HelicopterFlightSimulationPrivate::OnAlphaCMsg, this);
  this->node.Subscribe("/alpha_s", &HelicopterFlightSimulationPrivate::OnAlphaSMsg, this);
  ...
  this->controlJointRadToCL = _sdf->Get<double>(
                                      "control_joint_rad_to_cl", this->controlJointRadToCL)
                                  .first;
...
//////////////////////////////////////////////////
HelicopterFlightSimulation::HelicopterFlightSimulation()
    : System(), dataPtr(std::make_unique<HelicopterFlightSimulationPrivate>())
{
}

//////////////////////////////////////////////////
void HelicopterFlightSimulationPrivate::Update(EntityComponentManager &_ecm)
{
  GZ_PROFILE("HelicopterFlightSimulationPrivate::Update");
  ...
  // Calculate azimuth angle (Psi) based on the orientation of the blade
  double Psi = std::atan2(2.0 * (orientation.W() * orientation.Z() + orientation.X() * orientation.Y()),
                          1.0 - 2.0 * (orientation.Y() * orientation.Y() + orientation.Z() * orientation.Z()));

  {
    std::lock_guard<std::mutex> lock(this->alphaMutex);
    this->alpha0 = this->currentAlpha; // Use the subscribed angle of attack
  }

  // Calculate feathering angle theta(Psi) based on cyclic controls
  double thetaPsi = this->alpha0 + this->alphaC * std::cos(Psi) + this->alphaS * std::sin(Psi);

  // Use the updated feathering angle thetaPsi to compute new angle of attack (alpha)
  double alpha = thetaPsi;
  ...
  // should we keep this also for the cyclic control?
...
//////////////////////////////////////////////////
void HelicopterFlightSimulation::Configure(const Entity &_entity,
...
//////////////////////////////////////////////////
void HelicopterFlightSimulation::PreUpdate(const UpdateInfo &_info, EntityComponentManager &_ecm)
{
  GZ_PROFILE("HelicopterFlightSimulation::PreUpdate");
...
GZ_ADD_PLUGIN(HelicopterFlightSimulation,
                    System,
                    HelicopterFlightSimulation::ISystemConfigure,
                    HelicopterFlightSimulation::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(HelicopterFlightSimulation, "gz::sim::systems::HelicopterFlightSimulation")
